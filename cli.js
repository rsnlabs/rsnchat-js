...
function promptApiKey() {
  ...
  rl.question(colors.bold.green('Enter your API key: '), async (apiKey) => {
    try {
      const response = await axios.post('https://ai.rnilaweera.ovh/api/v1/user/validate', { key: apiKey });
...
function handleErrorResponse(status) {
  if (status === 403) {
    ...
    console.error(colors.red(`Unexpected response from the server. Status: ${status}. Please try again.`));
  }
  promptApiKey();
}
...
function startChat(rsnChat, username) {
  ...
  function promptChatMethod() {
    rl.question(colors.cyan('Select a chat method (GPT, BARD, OPENCHAT, LLAMA, GEMINI, MIXTRAL), or type "exit" to exit: '), (method) => {
      if (method.toLowerCase().trim() === 'exit') {
        console.log(colors.yellow('\nExiting RsnChat CLI. Goodbye!\n'));
        rl.close();
      } else if (['GPT', 'BARD', 'OPENCHAT', 'LLAMA', 'GEMINI', 'MIXTRAL'].includes(method.toUpperCase())) {
        promptUser(method.toUpperCase(), rsnChat);
      } else {
        console.error(colors.red('Invalid chat method. Please select GPT, BARD, OPENCHAT, LLAMA, GEMINI, MIXTRAL, or type "exit" to exit.'));
        promptChatMethod();
      }
    });
  }
  ...
  function promptUser(method, rsnChat) {
    rl.question(colors.green(`Enter your ${method} prompt (or type "exit" to exit): `), async (prompt) => {
